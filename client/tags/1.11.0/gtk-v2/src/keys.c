...
static void parse_keybind_line(char *buf, int line, int standard)
{
    ...
    if ((cpnext = strchr(buf,' '))==NULL) {
        LOG(LOG_WARNING, "gtk::parse_keybind_line",
    ...
    if (buf[0] == '!') {
        ...
        cp = strchr(cpnext, ' ');
        if (!cp) {
            LOG(LOG_WARNING, "keys.c:parse_keybind_line",
                "Line %d (%s) corrupted in keybinding file.", line,buf);
            return;
        }
    ...
    for (onkey=0; onkey<KEYHASH; onkey++) {
        for (key=keys[onkey]; key; key =key->next) {
            ...
            if (keyentry==count) {
                ...
                LOG(LOG_ERROR, "keys.c:unbind_key",
...
void keyfunc(GtkWidget *widget, GdkEventKey *event, GtkWidget *window) {
    ...
    if (!use_config[CONFIG_POPUPS]) {
        ...
        && (event->keyval == cancelkeysym)) {
            /* Player hit cancel button during input.
             * Disconnect it (code from menubar) */
