...
{
    EXISTCHECK(whoptr);
    return Py_BuildValue("i",*( int* )cf_object_get_property( whoptr->obj, CFAPI_OBJECT_PROP_COUNT));
}
...
{
    EXISTCHECK(whoptr);
    return Crossfire_Object_wrap(cf_object_get_property(whoptr->obj, CFAPI_OBJECT_PROP_ENEMY));
}
